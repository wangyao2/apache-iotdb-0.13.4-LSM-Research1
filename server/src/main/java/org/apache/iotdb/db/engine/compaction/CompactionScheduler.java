...
public class CompactionScheduler {
  ...
  public static void scheduleCompaction(TsFileManager tsFileManager, long timePartition) {
    if (!tsFileManager.isAllowCompaction()) {
      return;
    } // 选择和创建的任务都是通过compactionTaskManager的单例模式去提交的任务，在任务管理器里面有一个任务优先队列，负责任务的执行
    tryToSubmitCrossSpaceCompactionTask( //这里选择文件，创建合并任务之后，并不是按照这三个try去排列的优先级，而是按照优先级队列去排的
