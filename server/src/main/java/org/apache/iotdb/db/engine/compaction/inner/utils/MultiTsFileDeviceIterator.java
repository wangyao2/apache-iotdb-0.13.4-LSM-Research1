  ...
  public MultiTsFileDeviceIterator(List<TsFileResource> tsFileResources) throws IOException {
    ...
    try {
      for (TsFileResource tsFileResource : this.tsFileResources) {
        TsFileSequenceReader reader = new TsFileSequenceReader(tsFileResource.getTsFilePath()); //每一个TsFileSequenceReader的对象里面包装了一个FileChannel
        readerMap.put(tsFileResource, reader); //给每一个文件资源都创建一个文件阅读管道Channel
        deviceIteratorMap.put(tsFileResource, reader.getAllDevicesIteratorWithIsAligned());//给每一个文件资源,都创建一个里面所有设备的迭代器，通过阅读器，读取每一个资源文件内的所有设备信息
      }
  ...
  public class MeasurementIterator {
    ...
    private MeasurementIterator(
      ...
      if (needDeserializeTimeseries) {
        for (TsFileResource resource : tsFileResources) { //遍历每一个文件资源，去查找文件资源里面有没有跟当前设备匹配的序列
          TsFileSequenceReader reader = readerMap.get(resource);
          chunkMetadataIteratorMap.put(//返回每一个文件内，某个设备下所有序列的偏移量位置
