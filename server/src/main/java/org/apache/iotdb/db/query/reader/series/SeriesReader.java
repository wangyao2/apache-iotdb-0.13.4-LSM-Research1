...
public class SeriesReader {
  ...
  protected final QueryDataSource dataSource; //存储了查询过程中 涉及到的文件列表
   ...
   * page cache
   */
  protected VersionPageReader firstPageReader; //开始时间最小的 page reader
  protected final List<VersionPageReader> seqPageReaders = new LinkedList<>(); //当前获得的所有 page reader，按照每个 page 的起始时间进行排序
  protected final PriorityQueue<VersionPageReader> unSeqPageReaders;

  /*
   * point cache
   */
  protected final PriorityMergeReader mergeReader; //本质上是多个带优先级的 page，按时间戳从低到高输出数据点，时间戳相同时，保留优先级高的
...
//    String filePath = "RAoutput_DTDG_RoundOldTime_1_3MBWithRes5.csv";
