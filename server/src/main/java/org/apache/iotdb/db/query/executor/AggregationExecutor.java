...
public class AggregationExecutor {
  ...
  public QueryDataSet executeWithoutValueFilter(AggregationPlan aggregationPlan)
    ...
    //如果需要计算 count(s1), sum(s2), count(s3), sum(s1)，即需要计算 s1 的两个聚合值，那么将会得到 pathToAggrIndexesMap 结果为：s1 -> 0, 3; s2 -> 1; s3 -> 2
  ...
  private static void aggregateFromReader(
    ...
    while (seriesReader.hasNextFile()) {
      ...
      if (seriesReader.canUseCurrentFileStatistics()) { //确实，在聚合读取的时候，是有限通过统计量去分析
        ...
        continue;//如果能够使用当前文件的统计量，那么就跳过后面的chunk之类的判断，直接读取文件内容
      }

      while (seriesReader.hasNextChunk()) { //不能使用当前文件的 统计量，那么就解析里面的每一个chunk
        ...
        if (seriesReader.canUseCurrentChunkStatistics()) {
          ...
          continue; //当前块能用，那么就直接去看下一个块
        }

        remainingToCalculate = //当前块不能被读取，那么就去分析块内的Page
            aggregatePages(
