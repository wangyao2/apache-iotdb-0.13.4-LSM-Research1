...
public class RewriteCompactionFileSelector implements ICrossSpaceMergeFileSelector {
  ...
  private void selectOverlappedSeqFiles(TsFileResource unseqFile) throws IOException {
    ...
    TsFileDeviceInfo unseqFileDeviceInfo = new TsFileDeviceInfo(unseqFile); //把当前的unseq文件暂存一下
    for (DeviceInfo deviceInfo : unseqFileDeviceInfo.getDevices()) { //把当前的unseq文件解析成一个一个的设备,遍历里面的每一个设备
      String deviceId = deviceInfo.deviceId;
      long unseqStartTime = deviceInfo.startTime;
      long unseqEndTime = deviceInfo.endTime;

      boolean noMoreOverlap = false;
      for (int i = 0; i < resource.getSeqFiles().size() && !noMoreOverlap; i++) { //为每一个设备遍历每一个顺序文件
        ...
        if (!seqFile.isClosed()) { //对于未关闭的文件
          // for unclosed file, only select those that overlap with the unseq file//if time range in unseq file is 10-20,seq file is 15~?
        ...
        } else if (unseqEndTime <= seqEndTime) { //对于已经关闭的文件
        ...
        } else if (unseqStartTime <= seqEndTime) { //对于已经关闭的文件
