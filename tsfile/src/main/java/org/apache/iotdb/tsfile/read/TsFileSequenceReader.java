...
public class TsFileSequenceReader implements AutoCloseable {
  ...
  public TsFileDeviceIterator getAllDevicesIteratorWithIsAligned() throws IOException {
    ...
    Queue<Pair<String, Pair<Long, Long>>> queue = new LinkedList<>(); //queue里面的元素，分别是，序列名称，起始偏移量还有终止偏移量
  ...
  public Iterator<Map<String, List<ChunkMetadata>>> getMeasurementChunkMetadataListMapIterator(
    ...
    readFileMetadata(); //填充空的tsFileMetaData对象，读取文件的元数据

    MetadataIndexNode metadataIndexNode = tsFileMetaData.getMetadataIndex(); //获得metadata树
    Pair<MetadataIndexEntry, Long> metadataIndexPair =
        getMetadataAndEndOffset(metadataIndexNode, device, true, true);
    ...
    Queue<Pair<Long, Long>> queue = new LinkedList<>(); //前面是去解析一条序列在tsfile的位置
    ByteBuffer buffer = readData(metadataIndexPair.left.getOffset(), metadataIndexPair.right); //根据解析的位置去读取数据
    collectEachLeafMeasurementNodeOffsetRange(buffer, queue); //看上去是把所有的LeafMeasurement的起始偏移量和终止偏移量解析出来，
